...
option = st.selectbox("Input type", ["TEXT BOX", "CSV","PDF"])
df = None
if option == "CSV":
    ...
    train_file = st.file_uploader(
        'Upload a file', type=["pdf"]
    )
    embeddings = None
    if train_file is not None:
        # process_pdf_input(train_file)
        df, _, _, _ = process_pdf_input(train_file)
...
if df is not None:
    ...
    if advanced_options:
        ...
        MAX_TOKENS = st.slider(
            "max_tokens", min_value=1, max_value=2000, value=MAX_TOKENS
        )
